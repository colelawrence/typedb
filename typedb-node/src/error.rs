...
//! Error conversion from typedb-embedded errors to Node-API errors.

use napi::Error as NapiError;

use crate::types::{ErrorKind, ErrorLocation, NodeError};

/// Convert a typedb-embedded error to a NodeError.
pub fn convert_error(error: &typedb_embedded::Error) -> NodeError {
    ...
    NodeError { kind, message, location, hint }
}

/// Convert a typedb-embedded error to a napi::Error.
pub fn convert_error_to_napi(error: typedb_embedded::Error) -> NapiError {
