...
fn define_simple_entity() {
    let db = TestDatabase::new("test_simple_entity");
...
fn define_entity_with_attribute() {
    let db = TestDatabase::new("test_entity_attr");
    let schema = r#"
        define
...
fn define_entity_with_multiple_attributes() {
    let db = TestDatabase::new("test_entity_multi_attr");
    let schema = r#"
        define
        attribute name value string;
...
fn define_entity_hierarchy() {
    let db = TestDatabase::new("test_entity_hierarchy");
    let schema = r#"
        define
        attribute name value string;
        entity person owns name;
...
fn define_string_attribute() {
    let db = TestDatabase::new("test_string_attr");
    let result = db.define_schema("define attribute name value string;");
    assert!(result.is_ok(), "Failed to define string attribute: {:?}", result);
}

#[test]
fn define_integer_attribute() {
    let db = TestDatabase::new("test_int_attr");
    let result = db.define_schema("define attribute age value integer;");
    assert!(result.is_ok(), "Failed to define integer attribute: {:?}", result);
}

#[test]
fn define_double_attribute() {
    let db = TestDatabase::new("test_double_attr");
    let result = db.define_schema("define attribute score value double;");
    assert!(result.is_ok(), "Failed to define double attribute: {:?}", result);
}

#[test]
fn define_boolean_attribute() {
    let db = TestDatabase::new("test_bool_attr");
    let result = db.define_schema("define attribute active value boolean;");
    assert!(result.is_ok(), "Failed to define boolean attribute: {:?}", result);
}

#[test]
fn define_datetime_attribute() {
    let db = TestDatabase::new("test_datetime_attr");
    let result = db.define_schema("define attribute created_at value datetime;");
...
fn define_simple_relation() {
    let db = TestDatabase::new("test_simple_relation");
    let schema = r#"
        define
        entity person;
...
fn define_binary_relation() {
    let db = TestDatabase::new("test_binary_relation");
    let schema = r#"
        define
        entity person;
        entity company;
...
fn define_ternary_relation() {
    let db = TestDatabase::new("test_ternary_relation");
    let schema = r#"
        define
        entity person;
        entity project;
...
fn define_relation_with_attribute() {
    let db = TestDatabase::new("test_relation_attr");
    let schema = r#"
        define
...
fn define_attribute_cardinality() {
    let db = TestDatabase::new("test_attr_card");
    let schema = r#"
        define
        attribute name value string;
...
fn define_entity_with_key() {
    let db = TestDatabase::new("test_entity_key");
    let schema = r#"
        define
        attribute email value string;
        attribute name value string;
...
fn define_complex_schema() {
    let db = TestDatabase::new("test_complex_schema");
    let schema = r#"
        define
        # Attributes
        attribute name value string;
...
fn incremental_schema_definition() {
    let db = TestDatabase::new("test_incremental_schema");

    // First schema - base types
    db.define_schema(r#"
        define
        attribute name value string;
        entity person owns name;
    "#).expect("First schema commit failed");

    // Second schema - add more types
    db.define_schema(r#"
        define
        attribute age value integer;
        person owns age;
    "#).expect("Second schema commit failed");

    // Third schema - add relations
    db.define_schema(r#"
        define
        relation friendship relates friend;
        person plays friendship:friend;
    "#).expect("Third schema commit failed");
}
