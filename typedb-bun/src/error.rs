...
//! Error conversion from typedb-embedded errors to FFI errors.
...
pub fn error_from_message(message: &str) -> WasmError {
    WasmError {
...
pub fn error_invalid_handle(handle: u64) -> WasmError {
    WasmError {
        kind: ErrorKind::InternalError,
        message: format!("Invalid handle: {}", handle),
        location: None,
        hint: Some("Ensure the handle has not been dropped and belongs to this process.".to_string()),
    }
}
